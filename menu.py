...
class MainWindow(CTk):
    def __init__(self):
        ...
        self.add_message('Тестове повідомлення')
    ...
    def send_message(self):
        ...
        if message and not self.raw:
            ...
            data = f'' # тип повідомлення, ім'я користувача, повідомлення
            # try:
            ...
            self.add_message(f'{self.user_name}: {message}', img=CTkImage(Image.open(self.file_name),size=(300,300)))
            data = f'TEXT@{self.user_name}@{message}@{b64_data}' #тип повідомлення, ім'я користувача, повідомлення, зображення
